    ...
    try:
        ...
        if replace_host_files:
            ...
            for host_file, local_folder in host_files.items():
                ...
                msg = 'move %s to %s completed - done, out: %s' % (
                    remote_tmp_file, local_tmp_file, move_out)
                print_info(msg)
        else:
            print_info("Migration of avihost service not needed")
            return 2
    except subprocess.CalledProcessError as e:
        msg = 'Failed to replace avihost service files, error exception:%s' % str(
            e)
        print_error(msg)
        return 1
...
if __name__ == '__main__':
    ...
    try:
        ...
        traceback.print_exc()
        print_error(
            'Failed to migrate avihost service files, error exception:%s' % str(e))
        sys.exit(1)
    sys.exit(exitCode)
