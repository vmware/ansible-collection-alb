...
def patch_add_gslb(module, gslb_obj):
    ...
    if "dns_configs" in gslb_obj:
        ...
        gslb_obj["dns_configs"] = list(
            {v["domain_name"]: v for v in gslb_obj["dns_configs"]}.values()
        )
    else:
                    ...
                    for key, val in site.items():
                        if key == "dns_vses" and "dns_vses" in current_gslb_site:
                            ...
                            current_gslb_site["dns_vses"] = list(
                                {
                                    v["dns_vs_uuid"]: v
                                    for v in current_gslb_site["dns_vses"]
                                }.values()
                            )
                        else:
...
def main():
    argument_specs = dict(
        ...
        patch_level=dict(
            type="str", default="/site/dns_vses", choices=["/site/dns_vses", "/site"]
        ),
        async_interval=dict(
            type="int",
        ),
        ...
        enable_config_by_members=dict(
            type="bool",
        ),
        error_resync_interval=dict(
            type="int",
        ),
        ...
        send_interval_prior_to_maintenance_mode=dict(
            type="int",
        ),
