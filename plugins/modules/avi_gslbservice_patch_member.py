...
def delete_member(
    module, check_mode, api, tenant, tenant_uuid, existing_obj, data, api_version
):
    ...
    if existing_obj and (patched_member_ids or patched_member_fqdns):
        groups = [
            group
            for group in existing_obj.get("groups", [])
    ...
    if not group["members"]:
        ...
        existing_obj["groups"] = [
            grp
            for grp in existing_obj.get("groups", [])
            if grp["name"] != data["group"]["name"]
        ]
    # remove the members that are part of the list
    # update the object
    # added api version for AVI api call.
    rsp = api.put(
        "gslbservice/%s" % existing_obj["uuid"],
        data=existing_obj,
        tenant=tenant,
...
def add_member(
    module, check_mode, api, tenant, tenant_uuid, existing_obj, data, name, api_version
):
    rsp = None
    if not existing_obj:
        # create the object
        ...
        if check_mode:
            ...
            req = {"name": name, "groups": [data["group"]]}
            # added api version for AVI api call.
            rsp = api.post(
                "gslbservice",
                data=req,
        ...
        if "groups" not in req:
            req["groups"] = []
        groups = [
            group for group in req["groups"] if group["name"] == data["group"]["name"]
        ]
        if not groups:
            ...
            for patch_member in data["group"].get("members", []):
                ...
                for m in group["members"]:
                    if (
                        "fqdn" in patch_member
                        and m.get("fqdn", "") == patch_member["fqdn"]
                    ):
                        found = True
                        break
