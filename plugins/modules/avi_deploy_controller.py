...
def get_folder_by_path(si, dc, path):
    container = si.content.viewManager.CreateContainerView(dc, [vim.Folder], True)
    for managed_object_ref in container.view:
...
def get_cluster(si, dc, name):
    ...
    raise Exception("Failed to find cluster %s in datacenter %s" % (name, dc.name))
...
def is_update_cpu(module):
    return (
        "con_number_of_cpus" in module.params
        and module.params["con_number_of_cpus"] is not None
    )


def is_update_memory(module):
    return "con_memory" in module.params and module.params["con_memory"] is not None


def is_reserve_memory(module):
    return (
        "con_memory_reserved" in module.params
        and module.params["con_memory_reserved"] is not None
    )


def is_reserve_cpu(module):
    return (
        "con_cpu_reserved" in module.params
        and module.params["con_cpu_reserved"] is not None
    )


def is_resize_disk(module):
    return (
        "con_disk_size" in module.params and module.params["con_disk_size"] is not None
    )


def is_reconfigure_vm(module):
    return (
        is_update_cpu(module)
        or is_update_memory(module)
        or is_reserve_memory(module)
        or is_reserve_cpu(module)
        or is_resize_disk(module)
    )
...
def main():
    ...
    try:
        ...
        return module.fail_json(
            msg="exception while connecting to vCenter, check hostname, " "FQDN or IP"
        )
    ...
    if module.params["state"] == "absent":
        ...
        if vm is None:
            return module.exit_json(
                msg="A VM with the name %s not found" % (module.params["con_vm_name"])
            )

        if check_mode:
            return module.exit_json(
                msg="A VM with the name %s found" % (module.params["con_vm_name"]),
                changed=True,
            )
    ...
    if is_vm_exist(si, cl, module.params["con_vm_name"]):
        ...
        if vm_path != folder_path:
            ...
            changed = True
        if (
            not module.params["con_power_on"]
        ) and vm.runtime.powerState == vim.VirtualMachinePowerState.poweredOn:
            if not check_mode:
                task = vm.PowerOffVM_Task()
                wait_for_tasks(si, [task])
            changed = True
        if (
            module.params["con_power_on"]
            and vm.runtime.powerState == vim.VirtualMachinePowerState.poweredOff
        ):
            if not check_mode:
        ...
        if (
            module.params.get("con_mgmt_ip", None)
            and not module.params["con_mgmt_ip_v6_enable"]
        ):
            ip_addresses = get_vm_ips(vm)
            if ip_addresses and not module.params["con_mgmt_ip"] in ip_addresses:
                module.fail_json(msg="VM static ip address cant be modified")

        if (
            module.params.get("con_mgmt_ip_v6", None)
            and module.params["con_mgmt_ip_v6_enable"]
        ):
            ip_addresses = get_vm_ips(vm)
            if ip_addresses and not module.params["con_mgmt_ip_v6"] in ip_addresses:
                module.fail_json(msg="VM static ip address cant be modified")
        ...
        if is_reconfigure_vm(module):
            if not check_mode:
                ...
                if (
                    vmSummary.numCpu != module.params["con_number_of_cpus"]
                    or vmSummary.memorySizeMB != module.params["con_memory"]
                    or vmSummary.memoryReservation
                    != module.params["con_memory_reserved"]
                    or vmSummary.cpuReservation != module.params["con_cpu_reserved"]
                    or (
                        disk is not None
                        and disk.capacityInKB
                        != module.params["con_disk_size"] * 1024 * 1024
                    )
                ):
                    if vm.runtime.powerState == vim.VirtualMachinePowerState.poweredOn:
                        task = vm.PowerOffVM_Task()
                    ...
                    if is_reserve_cpu(module):
                        if (
                            vmSummary.cpuReservation
                            != module.params["con_cpu_reserved"]
                        ):
        ...
        if changed and not check_mode:
            module.exit_json(
                msg="A VM with the name %s updated successfully"
                % (module.params["con_vm_name"]),
                changed=True,
            )
    ...
    if module.params["con_ova_path"].startswith("http"):
        if (
            requests.head(
                module.params["con_ova_path"], verify=module.params["ssl_verify"]
            ).status_code
            != 200
        ):
            module.fail_json(
        ...
        if not os.path.isfile(module.params["con_ova_path"]) or not os.access(
            module.params["con_ova_path"], os.R_OK
        ):
    ...
    vi_string += "/%s%s/%s" % (dc.name, compile_folder_path_for_object(cl), cl.name)
    ...
    if (
        "ovf_network_name" in module.params.keys()
        and module.params["ovf_network_name"] is not None
        and len(module.params["ovf_network_name"]) > 0
    ):
        try:
            ...
            command_tokens.append(
                "--net:%s=%s"
                % (module.params["ovf_network_name"], module.params["con_mgmt_network"])
            )
    else:
        command_tokens.append("--network=%s" % module.params["con_mgmt_network"])
    ...
    if (
        ...
        command_tokens.append(
            "--prop:%s=%s" % ("avi.mgmt-ip.CONTROLLER", module.params["con_mgmt_ip"])
        )
    ...
    if (
        module.params.get("con_mgmt_mask", None)
        and not module.params["con_mgmt_ip_v6_enable"]
    ):
        command_tokens.append(
            "--prop:%s=%s"
    ...
    if (
        module.params.get("con_default_gw", None)
        and not module.params["con_mgmt_ip_v6_enable"]
    ):
        command_tokens.append(
            "--prop:%s=%s"
    ...
    if module.params.get("con_ovf_properties", None):
        for key in module.params["con_ovf_properties"].keys():
            command_tokens.append(
                "--prop:%s=%s" % (key, module.params["con_ovf_properties"][key])
            )

    if (
        "con_vcenter_folder" in module.params
        and module.params["con_vcenter_folder"] is not None
    ):
        command_tokens.append("--vmFolder=%s" % module.params["con_vcenter_folder"])
    ...
    if controller_ip:
        controller_up = controller_wait(
            controller_ip, module.params["round_wait"], module.params["con_wait_time"]
        )
