...
try:
    ...
    if (sdk_version is None) or (
        sdk_version and (parse_version(sdk_version) < parse_version("17.1"))
    ):
...
def main():
    ...
    path = "serviceengine"
    gparams = {"cloud_ref.name": module.params["se_cloud_name"]}
    ...
    while step < retries:
        ...
        rsp = api.get(
            ...
            tenant=module.params["se_tenant"],
            params=gparams,
            api_version=module.params["se_leader_ctl_version"],
        )
        rsp_data = rsp.json()
        for item in rsp_data["results"]:
            if (item["name"] == se_vm_name or item["name"] == se_mgmt_ip) and item[
                "se_connected"
            ]:
                my_deployed_se = item
                break
        if my_deployed_se is not None:
            mymsg = (
                "Service Engine '%s' is deployed and is connected to the Controller %s"
                % (my_deployed_se["name"], module.params["se_leader_ctl_ip"])
            )
            return module.exit_json(
                changed=True, msg=(mymsg), se_details=my_deployed_se
            )
        time.sleep(interval)
        step += 1
