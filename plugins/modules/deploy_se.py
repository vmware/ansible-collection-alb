...
def is_update_cpu(module):
    return (
        "se_vmw_number_of_cpus" in module.params
        and module.params["se_vmw_number_of_cpus"] is not None
    )


def is_update_memory(module):
    return (
        "se_vmw_memory" in module.params and module.params["se_vmw_memory"] is not None
    )


def is_reserve_memory(module):
    return (
        "se_vmw_memory_reserved" in module.params
        and module.params["se_vmw_memory_reserved"] is not None
    )


def is_reserve_cpu(module):
    return (
        "se_vmw_cpu_reserved" in module.params
        and module.params["se_vmw_cpu_reserved"] is not None
    )


def is_resize_disk(module):
    return (
        "se_vmw_disk_size" in module.params
        and module.params["se_vmw_disk_size"] is not None
    )
...
def main():
    ...
    if module.params["state"] == "absent":
        ...
        if vm is None:
            return module.exit_json(
                msg="A VM with the name %s not found"
                % (module.params["se_vmw_vm_name"])
            )

        if check_mode:
            return module.exit_json(
                msg="A VM with the name %s found" % (module.params["se_vmw_vm_name"]),
                changed=True,
            )
    ...
    if is_vm_exist(si, cl, module.params["se_vmw_vm_name"]):
        ...
        if vm_path != folder_path:
            ...
            changed = True
        if (
            not module.params["se_vmw_power_on"]
        ...
        if module.params.get("se_vmw_mgmt_ip", None):
            ...
            if ip_addresses and not module.params["se_vmw_mgmt_ip"] in ip_addresses:
    ...
    if (
        "se_vmw_ovf_networks" in module.params.keys()
    ...
    if module.params.get("se_vmw_mgmt_ip", None):
        command_tokens.append(
            "--prop:%s=%s" % ("avi.mgmt-ip.SE", module.params["se_vmw_mgmt_ip"])
        )

    if module.params.get("se_vmw_mgmt_mask", None):
        command_tokens.append(
            "--prop:%s=%s" % ("avi.mgmt-mask.SE", module.params["se_vmw_mgmt_mask"])
        )

    if module.params.get("se_vmw_default_gw", None):
        command_tokens.append(
            "--prop:%s=%s" % ("avi.default-gw.SE", module.params["se_vmw_default_gw"])
        )
    ...
    if module.params.get("se_vmw_ovf_properties", None):
        for key in module.params["se_vmw_ovf_properties"].keys():
            command_tokens.append(
                "--prop:%s=%s" % (key, module.params["se_vmw_ovf_properties"][key])
            )

    if (
        "se_vmw_vcenter_folder" in module.params
        and module.params["se_vmw_vcenter_folder"] is not None
    ):
        command_tokens.append("--vmFolder=%s" % module.params["se_vmw_vcenter_folder"])
