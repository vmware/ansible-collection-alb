...
class WS1loginSAMLApiSession(ApiSession):
    ...
    def saml_assertion(self, username, password):
        ...
        relay_state = re.search(
            WS1loginSAMLApiSession.request_relay_state_regex, resp.text, re.M | re.S).group(1)
        assertion_url = re.search(
            WS1loginSAMLApiSession.request_assertion_url_regex, resp.text, re.M | re.S).group(1)
        ...
        if "SAMLResponse" not in idp_resp.text:
            ...
            auth = {'username': username,
                    'password': password, 'issueToken': 'true'}
    ...
    def authenticate_session(self):
        ...
        try:
            ...
            relay_state = re.search(WS1loginSAMLApiSession.response_relay_state_regex, content, re.M |
                                    re.S).group(1)
            assertion_url = re.search(
                WS1loginSAMLApiSession.response_assertion_url_regex, content, re.M | re.S).group(1)
            saml_data = urllib.parse.urlencode({
