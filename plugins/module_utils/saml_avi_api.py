...
if ApiSession and sessionDict and APIError:
    class WS1loginSAMLApiSession(ApiSession):
        ...
        if requests:
            def saml_assertion(self, username, password):
                ...
                saml_request_match = re.search(WS1loginSAMLApiSession.saml_request_regex, resp.text, re.M | re.S)
        ...
        if ConnectionError and ChunkedEncodingError:
            def authenticate_session(self):
                ...
                try:
                    ...
                    headers = {'Content-Type': 'application/x-www-form-urlencoded'}
                    rsp = controller_session.post(assertion_url, headers=headers, data=saml_data)
    ...
    class OneloginSAMLApiSession(ApiSession):
        ...
        if requests:
            def saml_assertion(self, username, password):
                ...
                # Getting IDP session
                idp_session = requests.Session()
                saml_request_match = re.search(OneloginSAMLApiSession.saml_request_regex, resp.text, re.M | re.S)
                if not saml_request_match:
                    logger.error("SAML request not generated by controller.")
                    raise APIError("SAML request not generated by controller.")
                saml_request = saml_request_match.group(1)
                relay_state = re.search(OneloginSAMLApiSession.request_relay_state_regex, resp.text, re.M | re.S).group(1)
                assertion_url = re.search(OneloginSAMLApiSession.request_assertion_url_regex, resp.text, re.M | re.S).group(1)
                headers = {'Content-Type': 'application/x-www-form-urlencoded'}
                ...
                if self.idp_cookies:
                    ...
                    logger.info("Controller url %s generated SAML request is being sent to IDP.", saml_controller_url)
                    idp_resp = idp_session.post(assertion_url, headers=headers,
                                                data=saml_data,
        ...
        if ConnectionError and ChunkedEncodingError:
            def authenticate_session(self):
                ...
                try:
                    ...
                    relay_state = re.search(OneloginSAMLApiSession.response_relay_state_regex, content, re.M | re.S).group(1)
                    assertion_url = re.search(OneloginSAMLApiSession.response_assertion_url_regex, content, re.M | re.S).group(1)
                    saml_data = urllib.urlencode([
                        ('SAMLResponse', saml_response),
                        ('RelayState', relay_state)])
                    headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        ...
        if ConnectionError and ChunkedEncodingError:
            def authenticate_session(self):
                ...
                try:
                    ...
                    controller_session, resp = self.saml_assertion(username, password)
                    content = resp.text
                    saml_response_match = re.search(OktaSAMLApiSession.saml_response_regex, content, re.M | re.S)
                    saml_response = saml_response_match.group(1)
                    assertion_url = re.search(OktaSAMLApiSession.response_assertion_url_regex, content, re.M | re.S | re.IGNORECASE).group(1)
                    relay_state = re.search(OktaSAMLApiSession.response_relay_state_regex, content, re.M | re.S).group(1)
