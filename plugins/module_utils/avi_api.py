...
def avi_sdk_syslog_logger(
    logger_name="ansible_collections.vmware.alb.plugins.module_utils",
):
...
class AviCredentials(object):
    ...
    def update_from_ansible_module(self, m):
        ...
        if m.params["api_version"] and (m.params["api_version"] != "18.2.6"):
...
class ApiSession(Session):
    ...
    def __init__(
        ...
        if self.avi_credentials.controller.startswith("http"):
            ...
            if is_ipv6:
                self.prefix = "{}://[{}]".format(
                    protocol, self.avi_credentials.controller
                )
            else:
                self.prefix = "{}://{}".format(
                    protocol, self.avi_credentials.controller
        ...
        if self.avi_credentials.csrftoken:
            ...
            if self.avi_credentials.csp_host:
                if self.avi_credentials.csp_host.startswith("https"):
                    self.avi_credentials.csp_host = (
                        self.avi_credentials.csp_host.replace("https://", "")
                    )
    ...
    def get_session(
        ...
        if not idp_class:
            ...
            if not ("ApiSession" in str(idp_class.__base__)):
                raise APIError(
                    "idp_class {} not valid class. Please provide "
    ...
    def _api(
        ...
        **kwargs
        ...
        err = None
        api_hdrs = self._get_api_headers(
            tenant, tenant_uuid, timeout, headers, api_version
        )
        ...
        if "Authorization" not in api_hdrs:
            if self.pid != os.getpid():
                logger.info(
                    "pid %d change detected new %d. Closing session",
                    self.pid,
                    os.getpid(),
                )
                self.close()
            ...
            if "multipart/form-data" in api_hdrs["Content-Type"]:
                if connection_error:
                    ...
                    raise AviMultipartUploadError(
                        ...
                        "Msg %s" % (resp.status_code, resp.text),
                        resp,
                    )
            if connection_error:
                try:
                ...
                logger.info(
                    "received error %d %s so resetting connection",
                    resp.status_code,
                    resp.text,
                )
            if "Authorization" in api_hdrs:
    ...
    def get_object_by_name(
        ...
        **kwargs
        ...
        if resp.status_code > 499 or "Invalid version" in resp.text:
            logger.error(
                "Error in get object by name for %s named %s. "
    ...
    def _get_uuid_by_name(
        self, path, name, tenant="admin", tenant_uuid="", api_version=None
    ):
